...
because `jenkins` role allows it. A certificate's TTL issued using impersonation is limited the `max_session_ttl` of the
roles being impersonated.
...
- Alice can get a new `jenkins` certificate with the same TTL and some fields updated.
Teleport allows impersonated users to renew their certificates with with reduced the scope of the certificate.
...
We can make security-impersonator role more flexible by matching source user and target role parameters:
...
```bash
# Alice has to re-login to receive a new certificate with traits.
$ tsh login --proxy=teleport.example.com --user=alice --auth=local
# Alice can now get a security scanner
$ tctl --insecure auth sign --user=security-scanner --format=openssh --out security-scanner --ttl=10h
```

Here is a summary variables and functions we used in this guide:
