...
- Where will you host Teleport?
  - On-premises
  - Cloud VMs such as [AWS EC2](aws-oss-guide.mdx) or [GCE](gcp-guide.mdx)
...
source](installation.mdx#installing-from-source). Reference that guide to learn
...
2. The **ExecReload** command allows admins to run `systemctl reload teleport`.
   ...
   [etc 3.3](admin-guide.mdx#using-etcd) is configured**. Graceful Restarts will
...
Cluster](admin-guide.mdx#adding-nodes-to-the-cluster) which shows how to generate or set join tokens and
...
See [Teleport Proxy HA](admin-guide.mdx#teleport-proxy-ha) for more info.
...
the ownership of the directory with [`chown`](https://www.computerhope.com/unix/uchown.htm)
