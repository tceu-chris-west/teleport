...
// DefaultJitter is a global jitter instance used for one-off jitters.
// Prefer instantiating a new jitter instance for operations that require
// repeated calls.
var DefaultJitter = NewJitter()
...
// NewSmallJitter builds a new small jitter (currently jitters on
// the range [6n/7,n), but this is subject to change).  Prefer this over
// NewJitter() when jittering periodic operations (e.g. cert rotation
// checks) since large jitters result in significantly increased load.
func NewSmallJitter() Jitter {
	var mu sync.Mutex
	rng := rand.New(rand.NewSource(time.Now().UnixNano()))
	return func(d time.Duration) time.Duration {
		// values less than 1 cause rng to panic, and some logic
